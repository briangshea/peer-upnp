...
var INTERVAL = 10000;
var ROOT_TEMPLATE = fs.readFileSync(__dirname + '/../xml/root-desc.xml', 'utf8');
...
var RENDER_ROOT = ejs.compile(ROOT_TEMPLATE, {filename: __dirname + '/../xml/root-desc.xml'});
...
Peer.prototype.findDevice = function(udn){
  if( this.devices.hasOwnProperty(udn) ) {
    return this.devices[udn];
  } else {

    for( var d in this.devices ) {
      var device = this.devices[d].findDevice(udn);
...
Device.prototype.createDevice = function(options){
  options = options || {};
  options.root = false;
  var device = new Device(this.peer,options);
  this.devices[device.uuid] = device;

  return device;
}

Device.prototype.findDevice = function(udn) {
  if( this.devices.hasOwnProperty(udn) ) {
    return this.devices[udn];
  } else {
    for(var d in this.devices ) {
...
Service.prototype.set = function(name,value){
  ...
  if( this.description[name] === Object && this.description[name].hasOwnProperty("event") && this.description[name].event == true) {
    this.notify(name);
  }
};
...
var handleGetDeviceDescription = function(req,rsp,peer){
	...
	var device = peer.findDevice(udn);
	if (device) {
		var xml = RENDER_ROOT(device);
